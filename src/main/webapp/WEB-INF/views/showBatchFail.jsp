...
<body>
    ...
    <table class="bordered">
        ...
        <c:forEach items="${noSuccessSealBatchList}" var="item">
            <tr>
                <td>${fn:substring(item.parameters, 0, 100)}</td>
                <td>${item.time}</td>
                <td>${item.status}</td>
                <td><a target="_blank" href="<%=request.getContextPath()%>/sealResult/showBatchEdit?contractBatchId=${item.id}">编辑</a></td> 
            </tr>
        </c:forEach>
    </table>
</body>
